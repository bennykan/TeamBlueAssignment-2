...
We also noted there was 3 unknown values in Education, we decided to group them up into one unknown value
...
We would like to know the distributions of the numerical variables:
...
We noticed the variables Bill_AMT[N] and Pay_AMT[N] for N from 1 to 6 are related by the the Pay_AMT from the month is based on the Bill_AMT from the previous month.

```{r, message=FALSE,warning=FALSE}

data[1,colnames(data) %in% c("BILL_AMT3","PAY_AMT2")]

```

We created ratios to see how much of the previous month bill did the user pay off. We madesome assumptions where if the Bill Amount is negative then the ratio should be 1 has the bill has been over paided.


```{r, message=FALSE,warning=FALSE}


data$Ratio_2 <- ifelse(data$PAY_AMT1==0,0,ifelse(data$BILL_AMT2 <=0,1,data$PAY_AMT1/data$BILL_AMT2))
data$Ratio_3 <- ifelse(data$PAY_AMT2==0,0,ifelse(data$BILL_AMT3 <=0,1,data$PAY_AMT2/data$BILL_AMT3))
data$Ratio_4 <- ifelse(data$PAY_AMT3==0,0,ifelse(data$BILL_AMT4 <=0,1,data$PAY_AMT3/data$BILL_AMT4))
data$Ratio_5 <- ifelse(data$PAY_AMT4==0,0,ifelse(data$BILL_AMT5 <=0,1,data$PAY_AMT4/data$BILL_AMT5))
data$Ratio_6 <- ifelse(data$PAY_AMT5==0,0,ifelse(data$BILL_AMT6 <=0,1,data$PAY_AMT5/data$BILL_AMT6))
```
...
We are now able to create the K mean model, we are creating K = 6 clusters, we also use nstart = 20 to run the k means algorithm 20 times with random centroids each time and take the best result.
...
kmeans.result <- kmeans(df, centers=4,nstart = 20)
...
Paid <- ggplot(data = a_mn[a_mn$variable %in% c("Ratio_2","Ratio_3","Ratio_4","Ratio_5","Ratio_6"),], aes(x = variable, y = AvgValue, group = Cluster, fill = Cluster))
...
x <- ggplot(data = a_mn[!a_mn$variable %in% c("Ratio_2","Ratio_3","Ratio_4","Ratio_5","Ratio_6","AGE"),], aes(x = variable, y = AvgValue, group = Cluster, fill = Cluster))
...
Cluster 2 
  - Almost soley uses revolving credit
  - Around 55% Female
