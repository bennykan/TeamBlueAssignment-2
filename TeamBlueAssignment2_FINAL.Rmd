...
We cans see how different the outliers found by the LOF model is from the k means clusters model.  
...
Below are the Top 20 anomalies found by the LOF model.
...
kable(customer_anomaly_lof,caption="List of Customers Exhibiting LOF Anomaly Characteristics")
...
Now that we have identified some anomalies, we will need to go and investiagte each record to see which model was more effective at indicating default.  Once we get some feedback the anomalies we can back and adjust the data / model to better indicate default risk.  One option is to include more historical data, currently we only go back 6 months, could going back farther find more trends that relate to default risk.


#Deployment

We will deploy the model by sending Senior management the list of anomalies we currently found, we have also developed a application that allows the team to upload different data in the same format and run the model we built to find outliers of the data uploaded.
