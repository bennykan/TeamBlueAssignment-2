...
We can see that the average for all the clusters doesnt not vary too much.  They are all aroudn 35 to 36 years old.
...
We can see that cluster 3 and 4 tend to pay more of their bills then the other cluster. The only anonoly is in July for cluster 1, they had a spike in paying their bills.
...
From the above we can see that Cluster 1 and 2 have a significant higher averager bill then the other clusters.  Cluster 1 and 2 also have the lower limit balance. Cluster 2 and 3 have the highest average payment amount.
...
Looking at the vizualized data we can see some of the clusters are well formed and there is minimal overlap, clusters 2,3 and 4 are almost fully homogeneous from each other. Cluster 1 seems to be overlapping with parts of all the other clusters.
...
allows us to identify outliers in a data set that would not be outliers in another area of the data set. -->


```{r, message=FALSE,warning=FALSE}

#run LOF Model takes awhile 50 minutes 
outlier.scores <- lofactor(df,k=50)
...
We can see how different the outliers found by the LOF model is from the k-means clusters model.  
...
kable(customer_anomaly,caption="List of Customers Exhibiting Anomaly Characteristics using the K-Means Model")

```

#### LOF Model

We have also run the LOF model and below are the Top 20 anomalies found by the LOF model.

```{r, message=FALSE, warning=FALSE}

customer_anomaly_lof<-data[outliers_lof,!colnames(data) %in% c('ï..ID','Ratio_BILL_AUG','Ratio_BILL_JULY','Ratio_BILL_JUNE','Ratio_BILL_MAY','Ratio_BILL_APRIL')]
customer_anomaly_lof$Cluster<-outliers$cluster
kable(customer_anomaly_lof,caption="List of Customers Exhibiting Anomaly Characteristics using the LOF Model")

```

#Next Steps

Now that we have identified some anomalies, we will need to go and investigate each record to see which model was more effective at indicating default.  Once we get some feedback on the anomalies from the Retail Credit Risk and Collections departments, we can go back and adjust the data / model to better indicate default risks.  One option is to include more historical data as currently we only go back 6 months. More historical data could help us identify more accurate outliers.

#Deployment

We have deployed the model in an application format in which the users can upload the data file with a specified format. The application is tailored to the Retail Credit Risk and Collections departments who have the role to investigate credit card clients who have higher risk of default. The application can also be used by Senior Management as proof-of-concept. 

The users can specify the number of clusters and number of outliers they wish to see. Once these parameters are selected, the application will return a plot showing the outliers and the list of corresponding customers and their demographics as well as credit card bill and payment balances. 
